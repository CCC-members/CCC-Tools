...
load(connect_file);
if(exist('estimate_ec190','var'))
    connect = estimate_ec190;
    fig_title = 'Estimated Connectivity for Open Eyes';
else
    connect = matrix190_ec;
    fig_title = 'Estimated Connectivity for Closed Eyes';
end
% Preparing vars

surfaces = load(surf_file);
cortex = surfaces.Sc(surfaces.iCortex);
